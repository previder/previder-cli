...
func listSTaaSEnvironments(cmd *cobra.Command, args []string) error {
	...
	if outputType == "pretty" {
		...
		printTable([]string{"Id", "Name", "State", "Cluster", "ClusterId", "Type"}, intContent)
	} else {
		printJson(content)
	}
...
func createSTaaSEnvironment(cmd *cobra.Command, args []string) error {
	...
	createdStaasEnvironment, err := previderClient.STaaSEnvironment.Create(create)
	if err != nil {
		return err
	}
	if outputType == "pretty" {
		fmt.Printf("%+v\n", createdStaasEnvironment)
	} else {
		printJson(createdStaasEnvironment)
	}
	return nil
}
