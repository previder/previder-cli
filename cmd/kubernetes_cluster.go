...
func listKubernetesCluster(cmd *cobra.Command, args []string) error {
	...
	if outputType == "pretty" {
		...
		printTable([]string{"ID", "Name", "State", "Version"}, intContent)
	} else {
		printJson(content)
	}
	return nil
...
func createKubernetesCluster(cmd *cobra.Command, args []string) error {
	...
	createdKubernetesCluster, err := previderClient.KubernetesCluster.Create(create)
	if err != nil {
		return err
	}
	if outputType == "pretty" {
		fmt.Printf("%+v\n", createdKubernetesCluster)
	} else {
		printJson(createdKubernetesCluster)
	}
	return nil
}
